...
(function($) {
	...
	iPanorama.prototype = {
		...
		onSceneThumbsWheel: function(e) {
			...
			if( thumbIndex < 0 || this.sceneIdArray.length <= 1 ) {
				thumbIndex = 0;
			} else if( thumbIndex >= (this.sceneIdArray.length - 1) ) {
				thumbIndex = this.sceneIdArray.length - 1;
			}
		...
		moveToSceneThumb: function(index) {
			var sceneId = this.sceneIdArray[index],
		...
		refreshSceneThumb: function(sceneId) {
			if(this.controls.$sceneThumbs.hasClass("ipnrm-active")) {
				...
				var index = this.sceneIdArray.indexOf(sceneId);
				if(index >= 0) {
