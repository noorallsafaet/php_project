...
class FileManager extends CI_Controller {
	...
	public function index() {
		...
		if (Request::getApiParam('action') === 'rooturl') {
		...
		if (Request::getApiParam('action') === 'list') {
			...
			$this->config->load('myconfig');
			$dir = $this->config->item('myconfig_upload_path') . (strlen($path) ? '/' . $path : '');
			$link = base_url() . $dir;
				...
				foreach(scandir($dir) as $f) {
					...
					if(is_dir($dir . '/' . $f)) {
						$folders[] = array(
							...
							'type' => 'folder',
							'size' => 0,
							'created_at' => date('Y-m-d h:i:s',filemtime($dir . '/' . $f)),
		...
		if (Request::getApiParam('action') === 'rename') {
			...
			if(!file_exists($newDir)) {
				...
				$data['error'] = 'Error. Can\'t rename because another file exists with the same name';
			}
			...
			foreach ($items as $item) {
				...
				if (is_dir($dir_or_file)) {
					if(Helper::xdelete($dir_or_file)) {
						...
						$unprocessed[] = $item->id;
						$data['error'] .= 'Error. Can\'t delete the folder \'' . $item->name . '\', unknown error listing' . PHP_EOL;
					}
				} else {
		...
		if (Request::getApiParam('action') === 'mkdir') {
			...
			if(!file_exists($dir)) {
				if(@mkdir($dir)) {
					...
					$data['error'] = 'Error. Can\'t create the folder, unknown error listing';
				}
			} else {
				$data['error'] = 'Error. Can\'t create the folder because another folder exists with the same name';
			}
		...
		if (Request::getApiParam('action') === 'paste') {
			...
			$this->config->load('myconfig');
			$baseDir = $this->config->item('myconfig_upload_path');
				...
				if($operation === 'cut') {
					...
					if(!file_exists($newDir)) {
						if(@rename($oldDir, $newDir)) {
							...
							$unprocessed[] = $item->id;
							$data['error'] .= 'Error. Can\'t paste \'' . $name . '\', unknown error listing' . PHP_EOL;
						}
					} else {
						$unprocessed[] = $item->id;
						$data['error'] .= 'Error. Can\'t paste \'' . $name . '\', because another item exists with the same name' . PHP_EOL;
					}
					...
					if(!file_exists($newDir)) {
						...
						$unprocessed[] = $item->id;
						$data['error'] .= 'Error. Can\'t copy \'' . $name . '\', because another item exists with the same name' . PHP_EOL;
					}
				}
			}
				...
				foreach($files as $f) {
					...
					if (!file_exists($dst)) {
						if(@move_uploaded_file($src, $dst)) {
							...
							$data['result'] = false;
							$data['error'] .= 'Error. Can\'t upload the file \'' . $f['name'] . '\', unknown error listing' . PHP_EOL;
						}
					} else {
						$data['error'] .= 'Error. Can\'t upload the file \'' . $f['name'] . '\' because another file exists with the same name' . PHP_EOL;
					}
				}
			} else {
				$data['result'] = false;
