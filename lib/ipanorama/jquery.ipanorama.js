...
(function($) {
	...
	iPanorama.prototype = {
			...
			for (var key in this.config.scenes) {
				if (this.config.scenes.hasOwnProperty(key)) {
					...
					if (typeof scene.image === "string") {
						...
						images.push(scene.image.right);
						images.push(scene.image.left);
						images.push(scene.image.top);
						images.push(scene.image.bottom);
						images.push(scene.image.front);
						images.push(scene.image.back);
					}
				}
			}
		...
		loadImages: function(data) {
			if(data.images.length <= data.index)
				return;
			
			$.ajax({ 
				url : data.images[data.index],
				cache: true
			}).always(function(){
				data.index++;
